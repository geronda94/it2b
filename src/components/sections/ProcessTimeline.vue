...
// Конфигурация этапов в твоем стиле локализации
const steps = computed(() => [
  { 
    id: 1, 
    title: t.value?.process?.step1_title || 'Аудит и Аналитика', 
    desc: t.value?.process?.step1_desc || 'Погружаемся в ваши бизнес-процессы, находим "узкие горлышки" и проектируем гибридную архитектуру, которая сэкономит вам месяцы разработки.' 
  },
  { 
    // Звучит как у дорогих интеграторов
    id: 2, 
    title: t.value?.process?.step2_title || 'Архитектура и Proof of Concept', 
    desc: t.value?.process?.step2_desc || 'Проектируем структуру баз данных и логику процессов. Собираем базовый каркас системы и проводим первую демонстрацию. После утверждения концепта запускаем полномасштабную разработку.' 
  },
  { 
    id: 3, 
    title: t.value?.process?.step3_title || 'Разработка и Автоматизация', 
    desc: t.value?.process?.step3_desc || 'Комбинируем чистый код с мощными Low-Code инструментами (Directus, n8n). Настраиваем API, CRM и сложную логику прозрачными спринтами с регулярными демо.' 
  },
  { 
    id: 4, 
    title: t.value?.process?.step4_title || 'Релиз и Развитие', 
    desc: t.value?.process?.step4_desc || 'Бесшовно разворачиваем систему на боевых серверах (DevOps). Обучаем вашу команду, забираем проект на мониторинг 24/7 и планируем масштабирование.' 
  }
])
