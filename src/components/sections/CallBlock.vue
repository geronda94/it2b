              ...
              <form @submit.prevent="handleSubmit" class="flex flex-col gap-5">
                ...
                <div class="relative group">
                  ...
                  <input 
                    ...
                    required
                    :placeholder="getT('cta.tg_placeholder', '@username или телефон')"
                ...
                >
                  ...
                  <div class="relative z-10 flex items-center gap-2">
                    <span v-if="!isSubmitting && !isSuccess">{{ getT('cta.btn', 'Start Project') }}</span>
                    <span v-else-if="isSubmitting">{{ getT('cta.btn_submitting', 'Отправка...') }}</span>
                    <span v-else>{{ getT('cta.btn_success', 'Заявка принята!') }}</span>
                    
                    <font-awesome-icon v-if="!isSubmitting && !isSuccess" :icon="['fas', 'arrow-right']" class="transition-transform duration-300 group-hover:translate-x-1" />
                    <font-awesome-icon v-if="isSubmitting" :icon="['fas', 'circle-notch']" class="animate-spin" />
                    <font-awesome-icon v-if="isSuccess" :icon="['fas', 'check']" />
                  </div>
                </button>

                <p class="text-[10px] text-center text-[var(--text-secondary)] mt-2">
                  {{ getT('cta.privacy_text', 'Нажимая на кнопку, вы соглашаетесь с') }} 
                  <a href="#" class="underline hover:text-white transition-colors">{{ getT('cta.privacy_link', 'политикой конфиденциальности') }}</a>.
                </p>
